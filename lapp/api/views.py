...
from .serializers import SingaporeLastDigitSerializer, SingaporeLastTwoDigitSerializer, SingaporeLastThreeDigitSerializer, DearLastDigitSerializer, DearLastTwoDigitSerializer, DearLastThreeDigitSerializer
...
def latest_apk(request):
    apk = AppRelease.objects.filter(is_active=True).order_by('-uploaded_at').first()

    if not apk:
        return Response({"error" : "No APK available"}, status=status.HTTP_404_FORBIDDEN)
    return Response({
        "version" : apk.verison,
